...
const DEMO_SAMPLES = [
  ...
  },
  {
    id: 'tom',
    name: 'Tom',
    description: 'Tom medio',
    url: 'https://tonejs.github.io/audio/drum-samples/CR78/tom1.mp3',
    color: '#1db954',
  },
  {
    id: 'clap',
    name: 'Clap',
    description: 'Palma clap',
    url: 'https://tonejs.github.io/audio/drum-samples/Techno/clap.mp3',
    color: '#5865F2',
  },
  {
    id: 'perc',
    name: 'Perc',
    description: 'Percussao',
    url: 'https://tonejs.github.io/audio/drum-samples/Techno/perc.mp3',
    color: '#a0522d',
  },
]
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    switch (type) {
      case 'sawtooth':
        ...
        break
      case 'square':
        sample = ((t * freq) % 1) < 0.5 ? 0.8 : -0.8
        break
      case 'sine':
        sample = Math.sin(2 * Math.PI * t * freq)
        break
      case 'triangle':
        sample = 4 * Math.abs((t * freq) % 1 - 0.5) - 1
        break
    }
  ...
  return (
    <aside className="source-browser panel-card">
      ...
      <div className="demo-samples">
        ...
        <p className="demo-desc">Samples basicos do Tone.js para comecar.</p>
        <div className="demo-grid">
          {DEMO_SAMPLES.map((demo) => (
            <button
              key={demo.id}
              className="demo-btn"
              onClick={() => void handleAddDemoSample(demo)}
              disabled={loadingId !== null}
              style={{ '--demo-color': demo.color } as React.CSSProperties}
            >
              <span className="demo-name">{demo.name}</span>
              <span className="demo-desc-small">{demo.description}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="synth-section">
        ...
        <p className="synth-desc">Osciladores basicos para bass, lead, pad e arpejo.</p>
        <div className="synth-grid">
          {SYNTH_PRESETS.map((preset) => (
            <button
              key={preset.id}
              className="synth-btn"
              onClick={() => void handleAddSynth(preset)}
              disabled={loading}
              style={{ '--synth-color': preset.color } as React.CSSProperties}
            >
              <span className="synth-name">{preset.name}</span>
              <span className="synth-freq">{preset.freq}Hz</span>
            </button>
          ))}
        </div>
      </div>
